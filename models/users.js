    ...
    if (newPassword === newPassword1) {
        if (oldPasdword != newPassword) {
            db.User.findOneAndUpdate({
                ...
                Password: hash(oldPasdword)
            }, {
                Password: hash(newPassword)
            }, (err, response) => {
                if (err) {
                    res.render('changepassword', {
                        message: "Somethig go wrong. Try one more time."
                    });
                } else res.redirect('/user/myprofile');
            });
        } else res.render('changepassword', {
            message: "New password can't be the same as old"
        });
    } else res.render('changepassword', {
...
exports.updateEmail = function (req, res) {
    if (req.oldEmail != req.newEmail) {
        ...
        res.render('changeemail', {
            message: "New email adress cat be teh same as old"
        });
    }
}
    ...
    db.Token.findOne({
        ...
        db.User.findOne({
            ...
            if (!user) return res.status(400).send({
                type: 'not_verified',
                msg: "We ar unabele to find a user for this token"
            });
            ...
            user.save((err) => {
                ...
                res.render('login', {
                    message: "The account has been verified log in"
                });
            });
        });
    });
...
exports.resendTokenPost = function (req, res, next) {
    db.User.findOne({
        ...
        if (!user) return res.status(400).send({
            msg: "We are unable to find a user"
        });
        if (user.isVerified) return res.render('login', {
            message: "The account has been verified log in"
        });
...
exports.retrievePassword = function (req, res) {
    db.User.findOne({
        ...
        if (!user) return res.status(400).send({
            msg: "We are unable to find user"
        });
