...
exports.createList1 = function(req, res){
    ...
    var sameURL = true;
    db.List.findOne({ListURL: url}, (err, response) =>{
        if(err)res.render('show_message', {message: "Databese error", type: "error"});
...
exports.renderListUser = async function(req, res){
    ...
    var id
    var name
    await db.List.findOne({ListURL: req.params.id}, async (err, response) =>{
        if(err)res.render('show_message', {message: "Databese error", type: "error"});
    ...
    new Promise(function (resolve, reject){
        db.ListItem.find({ListID: id}).cursor()
        .on ('data', function(doc) { items1[doc.ListItemPosition]= doc.ListText })
        .on('error',reject)
        .on('end',resolve );
    })
   ...
   .then(() =>{
    ...
    res.render('list', {title: name, items: items1, url:req.params.id})
})
}
...
exports.renderList = async function(req, res){
   ...
   .then(() =>{
    res.render('list1', {title: name, items: items1, url:req.params.id})
})
}
...
exports.deleteList = async function(id, req, res){
    try{
 db.List.findOneAndRemove({ListURL: id}, async (err, response) =>{
